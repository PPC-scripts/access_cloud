...
## testing for rclone and, in a debian system, try to install it if user so selects
	if ! [ -x "$(command -v rclone)" ]; then
	# yad --title="Access Cloud Storage- Error" --window-icon=/usr/share/icons/papirus-antix/48x48/places/folder-red-meocloud.png --center --text=" Please install rclone using your Package Manager! " --button="Close"
	yad --title="Access Cloud Storage" --window-icon=/usr/share/icons/papirus/48x48/places/folder-red-meocloud.png --center --text=" rclone needs to be installed. \n Do you want to automaticaly try to install it (you'll have to relaunch this application)? " --button="Install rclone":1 --button="Close":2	
	 foo=$?
		if [[ $foo -eq 1 ]]; then x-terminal-emulator -e gksudo apt install rclone
		fi
	  exit 1
	fi
...
Manage_remotes(){
	...
	yad --title="Access Cloud Storage - Configuration" --window-icon=/usr/share/icons/gnome/48x48/places/gnome-fs-share.png --center --text="\n Use the buttons bellow to configure a 'remote' Cloud account \n You can access an on-line example of how to manually configure a Google Drive account from the Help window. \n  " --button="Manage Cloud Drives":'x-terminal-emulator -e rclone config' --button="Automatically add Google Drive":'x-terminal-emulator -e rclone config create GoogleDrive drive config_is_local false' --button="Tray icon":3 --button="Help":"bash -c Help" --button="Close":4
	
	 foo=$?
		if [[ $foo -eq 3 ]]; then
			tray=$(yad  --title="Access Cloud Storage - Configuration" --window-icon=/usr/share/icons/gnome/48x48/places/gnome-fs-share.png --form --center --field 'Use Tray icon - NOTE: Requires restart!':chk true) && echo $tray > /tmp/rclone_tray.txt
		fi
...
file=~/.rclone_mountpoint_list.conf
##now=`date +"%Y-%m-%d"`
# Choose an existing "remote" cloud service using yad (or unmount or manage "remotes")
 rclone listremotes > /tmp/rclone_remote_list.txt
 log_file="/tmp/rclone_remote_list.txt"
 selection=$(yad --title="Access Cloud Storage" --window-icon=/usr/share/icons/gnome/48x48/places/gnome-fs-share.png --width=550 --height=400 --center --separator=" " --list  --column=" Double click the 'remote' Cloud service that you want to access "  --button="Unmount Drives!!Unmounts all 'remote' cloud drives":1 --button="Configuration!!Allows to manage cloud drives and the tray icon":2 --button="Help":"bash -c Help" --button="Quit!gtk-quit!Closes the application and its tray icon":3 < /tmp/rclone_remote_list.txt)
 foo=$?
...
if [[ $foo -eq 1 ]]; then
	...
	pkill rclone
    # Try to delete empty mountpoint directories listed in log file (called "file"):
#file=~/.rclone_mountpoint_list.conf
#touch $file
lines=`cat $file`
...
( echo $mount_point_no_spaces ; echo "" ) >> $file
...
#show the cloud service contents on thunar
 thunar "$mount_point_no_spaces"  
...
if grep -Fxq $tray /tmp/rclone_tray.txt ;then
    pkill yad
yad --notification --image="/usr/share/icons/gnome/48x48/places/gnome-fs-share.png" --command="bash -c Main_window" --text="Left click: Access Cloud Storage (Middle click: exits)"
else
