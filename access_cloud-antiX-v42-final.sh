...
# icon for antiX:
declare -x icon="/usr/share/icons/papirus-antix/48x48/places/folder-red-meocloud.png"
#icon for MX linux:
# declare -x icon="/usr/share/icons/gnome/48x48/places/gnome-fs-share.png"
Initial_check(){
...
## testing for rclone and, in a debian system, try to install it if user so selects
 if ! [ -x "$(command -v rclone)" ]; then
	# yad --title="Access Cloud Storage- Error" --window-icon=$icon --center --text=" Please install rclone using your Package Manager! " --button="Close"
	yad --title="Access Cloud Storage" --window-icon=/usr/share/icons/papirus-antix/48x48/places/folder-red-meocloud.png --center --text=" rclone needs to be installed. \n Do you want to automaticaly try to install it (you'll have to relaunch this application)? " --button="Install rclone":1 --button="Close":2	
	  foo=$?
	  [[ $foo -eq 1 ]] && x-terminal-emulator -e gksudo apt install rclone
	exit 1
 fi
...
#Show traybar icon (if user checked the Tray checkbox, in the main window, on by default): (the pkill yad garantees that there are no multiple instances of the icon on the toolbar, unfortunatly, also closes any other running yad window (TO-DO: find another way to avoid multiple instances) 
 tray="TRUE|"
  if grep -Fxq $tray /tmp/rclone_tray.txt ;then
    pkill yad
    yad --notification --image=$icon --command="bash -c Main_window" --text="Left click: Access Cloud Storage (Middle click: exits)"
      else
...
# Choose an existing "remote" cloud service using yad (or unmount or manage "remotes")
 rclone listremotes > /tmp/rclone_remote_list.txt
 log_file="/tmp/rclone_remote_list.txt"
 selection=$(yad --title="Access Cloud Storage" --window-icon=$icon --width=550 --height=400 --center --separator=" " --list  --column=" Double click the 'remote' Cloud service that you want to access "  --button="Unmount Drives!!Unmounts all 'remote' cloud drives":1 --button="Configuration!!Manage cloud drives and the tray icon":2 --button="Help":"bash -c Help" --button="Quit!gtk-quit!Closes the application and its tray icon":3 < /tmp/rclone_remote_list.txt)
 foo=$?
...
# add name of directory to a config file, to be able to, when unmounting, also delete the empty mount point folder
 ( echo $mount_point_no_spaces ; echo "" ) >> ~/.rclone_mountpoint_list.conf
...
#show the cloud service contents on the default file manager: for other distros other than antiX, that have xdg-open installed uncoment the following line and comment the "desktop-defaults" one...
 # cd ~ && thunar $mount_point_no_spaces &
  desktop-defaults-run -fm ~/$mount_point_no_spaces
} #end of 'Main_window' funtion
 
#Start Script:
 export -f Help Manage_remotes Initial_check Main_window Tray_icon
 Initial_check
 Main_window
 Tray_icon
