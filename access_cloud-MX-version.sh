...
## testing for  yad
	if ! [ -x "$(command -v yad)" ]; then
	 ...
	 OPTIONS=(1 "You need to have YAD installed!")
     CHOICE=$(dialog --clear --title "Access_cloud" --menu "Warning:" 15 60 4 "${OPTIONS[@]}" 2>&1 >/dev/tty)
	 clear && exit 1
	fi
## testing for rclone and, in a debian system, try to install it if user so selects
	...
	yad --title="Access Cloud Storage" --window-icon=/usr/share/icons/papirus-antix/48x48/places/folder-red-meocloud.png --center --text=" rclone needs to be installed. \n Do you want to automaticaly try to install it (you'll have to relaunch this application)? " --button="Install rclone":1 --button="Close":2	
	 foo=$?
		if [[ $foo -eq 1 ]]; then x-terminal-emulator -e gksudo apt install rclone -y
		fi
	  exit 1
	fi
...
x-terminal-emulator -e fusermount -u ~/"${line}" 
rmdir ~/"${line}"
done
	# Delete log file
###EXPERIMENTAL:
...
touch ~/.rclone_mountpoint_list.conf
